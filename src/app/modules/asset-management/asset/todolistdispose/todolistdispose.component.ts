...
@Component({
  selector: 'app-todolistdispose',
  templateUrl: './todolistdispose.component.html',
  styleUrls: ['./todolistdispose.component.scss']
})
export class TodolistdisposeComponent implements OnInit {
  to = 0;
  from = 0;
  pageSize = 10;
    ...
    if (ev.target.checked == true) {
      this.rows.forEach((ele, index) => {
        this.allSeletectedAsset.push({
          ...
          AssetId: ele.id,
          MasterWorkflowId: ele.masterWorkflowId,
          MasterDisposeTransactionId: ele.masterDisposeTransactionId,
        });
      });
    } else {
      this.allSeletectedAsset = [];
  ...
  getTodolistChecked(isSelected: any, asset: any) {
    ...
    if (isSelected == true) {
      this.allSeletectedAsset.push({
        ...
        AssetId: asset.id,
        MasterWorkflowId: asset.masterWorkflowId,
        MasterDisposeTransactionId: asset.masterDisposeTransactionId,
      });
    } else {
  ...
  viewHandler(event, data: any) {
    ...
    this.router.navigate(['/asset-management/asset/viewassetprofiledispose']);
  }
  ...
  requestHandlerSelectedAll(remark: any, actionStatus: any) {
    ...
    //console.log("requestData", requestData)
    this.workflowService
      .postUpdateMultiDisposeReviewApproverProcess(requestData)
      .subscribe(
        (res: any) => {
          this.success(res);
          this.checkResult = true;
