...
@Component({
  selector: 'app-preventive-month-year-wise',
  templateUrl: './preventive-month-year-wise.component.html',
  styleUrl: './preventive-month-year-wise.component.scss'
})
export class PreventiveMonthYearWiseComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    this.currentIndex = sessionStorage.getItem("PreventiveCurrentIndexWorkingHoursTop") ? Number(sessionStorage.getItem("PreventiveCurrentIndexWorkingHoursTop")) : 0;
  ...
  Get_V3_GetTotalWorkingHours12Month_Preventive() {
    let url = 'api/MaintentDash/Get_V3_GetTotalWorkingHours12Month_Preventive'
  ...
  Get_V3_GetTotalWorkingHoursDetailList_Preventive(yearMonth) {
    ...
    let url = 'api/MaintentDash/Get_V3_GetTotalWorkingHoursDetailList_Preventive'
  ...
  Get_V3_GetTotalWorkingHoursByTech_Preventive(yearMonth) {
    ...
    let url = 'api/MaintentDash/Get_V3_GetTotalWorkingHoursByTech_Preventive'
    this.commonHttpServiceCallerService.postWithJsonDataMethod(url, paylod).subscribe((res: any) => {
  ...
  callAfterClickYearMonth(index) {
    ...
    sessionStorage.setItem("PreventiveCurrentIndexWorkingHoursTop", "" + index)
