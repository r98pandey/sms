...
@Component({
  selector: 'app-corrective-month-year-wise',
  templateUrl: './corrective-month-year-wise.component.html',
  styleUrl: './corrective-month-year-wise.component.scss'
})
export class CorrectiveMonthYearWiseComponent implements OnInit, OnChanges {
  ...
  Get_V3_GetTotalWorkingHours12Month_Corrective() {
    let url = 'api/MaintentDash/Get_V3_GetTotalWorkingHours12Month_Corrective'
  ...
  ngOnInit(): void {
    this.currentIndex = sessionStorage.getItem("CorrectiveCurrentIndexWorkingHoursTop") ? Number(sessionStorage.getItem("CorrectiveCurrentIndexWorkingHoursTop")) : 0;
  ...
  Get_V3_GetTotalWorkingHoursDetailList_Corrective(yearMonth) {
    ...
    let url = 'api/MaintentDash/Get_V3_GetTotalWorkingHoursDetailList_Corrective'
  ...
  Get_V3_GetTotalWorkingHoursByTech_Corrective(yearMonth) {
    ...
    let url = 'api/MaintentDash/Get_V3_GetTotalWorkingHoursByTech_Corrective'
    this.commonHttpServiceCallerService.postWithJsonDataMethod(url, paylod).subscribe((res: any) => {
  ...
  callAfterClickYearMonth(index) {
    ...
    sessionStorage.setItem("CorrectiveCurrentIndexWorkingHoursTop", "" + index)
