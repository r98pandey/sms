...
@Component({
  selector: 'app-request-work-inspection',
  templateUrl: './request-work-inspection.component.html',
  styleUrl: './request-work-inspection.component.scss'
})
export class RequestWorkInspectionComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    console.log("projectProcessId==", this.projectProcessId)
    this.getfromBinding();
  ...
  openModalMember(projectProcessHeaderDocId: any) {
  ...
  openModaAfterGetprojectProcessHeaderDocId(projectProcessHeaderDocId) {
    ...
    modalRef.componentInstance.title = 'Your project process has been successfully created. You are now ready to upload the document.';
    modalRef.componentInstance.subTitle = "";
    ...
    modalRef.result.then((result) => {
      ...
      if (result) {
        if (result == "Close click") {
          this.openModalMember(projectProcessHeaderDocId)
        }
      }
    });
  }
