...
@Component({
  selector: 'app-attmachment-agreement',
  templateUrl: './attmachment-agreement.component.html',
  styleUrl: './attmachment-agreement.component.scss'
})
export class AttmachmentAgreementComponent implements OnInit {
  @Input() projectId: any;
  @Input() maintenanceId: any;
  @Input() dDetail:any
@Input()maintenanceStatusId:any;
@Input() maintenanceStatusName:any;
  ...
  openModalUploadeAttachments() {
    console.log("MaintenanceId", this.maintenanceId, this.projectId)

    const modalRef = this.modalService.open(AddUploadMainateaanceAttachmentComponent, {
      ariaLabelledBy: "modal-basic-title",
      size: "md",
      centered: true,
      backdrop: "static",
    ...
    modalRef.componentInstance.maintenanceId = this.maintenanceId;
  ...
  openModalAfterCanceledUpdloadDelete(type) {
    ...
    modalRef.componentInstance.title = 'You cannot '+ type +'  document because the maintenance agreement is under  '+ this.maintenanceStatusName +' .';
    modalRef.componentInstance.subTitle = "";
