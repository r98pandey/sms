...
@Component({
  selector: 'app-project-work-progress-view-with-task',
  templateUrl: './project-work-progress-view-with-task.component.html',
  styleUrl: './project-work-progress-view-with-task.component.scss'
})
export class ProjectWorkProgressViewWithTaskComponent implements OnInit {
  activeId = 1;
  ...
  breadCrumbItems: any = [
    ...
    { label: "Work Programme View", active: true },
  ];
  ...
  constructor(
    ...
    if (this.projectScheduleService.sendProjectScheduleId == 0) {
      this.router.navigate(["/project-management/work-progress/work-progress-list"]);
    } else {
  ...
  createTask(): void {
    if (this.storeProjectScheduleObject.projectScheduleStatusId == 49) {
      ...
      const modalRef = this.modalService.open(AddTaskModalWorkProgressComponent, {
        ariaLabelledBy: "modal-basic-title",
        size: "lg",
        centered: true,
        backdrop: "static",
  ...
  openViewHandleTask(projectTaskId, projectScheduleObject): void {
    const modalRef = this.offcanvasService.open(ProjectWorkProgressTaskDetailsComponent, {
      scroll: false,
      position: "end",
      keyboard: false,
      backdrop: "static",
