...
export class ConfigurationComponent implements OnInit {
  breadCrumbItems: any = [
    { label: "Workflow" },
    { label: "Workflow Settings", active: true },
  ];
  label: any = "Workflow";

  workflowDetailsObject: any = {};
  _globalCompanyId: any;
  _globalDepartmentId: any;
  _globalDepartmentName: any;
  ...
  constructor(
    ...
    this.workflowDetailsObject = { ...this.workflowService.wfItem };
    this._globalDepartmentId = this.workflowDetailsObject.departmentId;
    this._globalCompanyId = this.workflowDetailsObject.companyId;
    this._globalDepartmentName = this.workflowDetailsObject.departmentName;

    if (
      this.workflowDetailsObject.departmentId == 0 ||
      this.workflowDetailsObject.departmentId == null
    ) {
      this.router.navigate([
        "application-settings/workflow-setup/listworkflow",
      ]);
    } else {
      this.getWorkflowDetails(this._globalDepartmentId);
      this.getMasterApproverGroupTypeList();
      // this.getUserDetails(this._globalCompanyId)
    }
  }
  ngOnInit(): void { }
  /**
  ...
  sendConfigDetails(config: any) {
    ...
    this.getUserListByCompany(workflowUrl ? workflowUrl.url : null, this._globalDepartmentId);
  ...
  submitInsertWorkflowUser() {
    ...
    this.approverTypeByGroupTypeIdList.forEach((ele) => {
      this.requestData.push({
        DepartmentId: this._globalDepartmentId,
        UserId: ele.userId,
