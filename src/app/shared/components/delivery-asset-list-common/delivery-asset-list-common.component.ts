...
@Component({
  selector: "app-delivery-asset-list-common",
  templateUrl: "./delivery-asset-list-common.component.html",
  styleUrls: ["./delivery-asset-list-common.component.scss"],
})
export class DeliveryAssetListCommonComponent
  implements OnInit, OnChanges, AfterViewInit
{
  ...
  onTypePoNumberChange() {
    //console.log(this.typePoNumber);
    this.page = 1;
    this.payload.displayStart = this.pageSize * (this.page - 1);
    this.payload.SearchPoNumber = this.typePoNumber;
    this.loadData();
  }
  ...
  @ViewChild("poNumber", { static: true }) poNumber: ElementRef;
  ngAfterViewInit() {
    ...
    fromEvent(this.poNumber.nativeElement, "input")
      .pipe(
        filter(Boolean),
        debounceTime(1000),
        distinctUntilChanged(),
        tap((event: KeyboardEvent) => {
          this.onTypePoNumberChange();
        })
      )
      .subscribe();
  }
