...
@Component({
  selector: "app-task-end-audit",
  templateUrl: "./task-end-audit.component.html",
  styleUrls: ["./task-end-audit.component.scss"],
})
export class TaskEndAuditComponent implements OnInit, OnChanges {
  @Input() ticketWOStartEndTask: any = [];

  @Input() workorderId: any;
  @Input() masterAuditList: any;
  @Input() expectedComplitionDateTime: boolean = true;
  @Output() afterSubmitAudit = new EventEmitter();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    console.log("this.masterAuditList", this.masterAuditList)
    ...
    if (this.authAssetService.getRole() !== "Client User") {
      ...
      if (
        this.masterAuditList?.auditStatusId == 29 ||
        this.masterAuditList?.auditStatusId == 30 ||
        this.masterAuditList?.auditStatusId == 34 ||
        this.masterAuditList?.auditStatusId == 2
...
checkStartStarus(auditStatusId:any){
  let showButtons = false;
  if ([2, 30, 34].includes(parseInt(auditStatusId))) {
    showButtons = true;
  }
  if (
    [61].includes(parseInt(auditStatusId)) &&
  ...
  getTechnitionAttendanceListByTaskForAudit(WOTaskId) {
    let data = {
      WOTaskId: WOTaskId,
    };
    this.auditService
    .getTechnitionAttendanceListByTaskForAudit(data)
      .subscribe((res: any) => {
  ...
  updateServiceOrderStatusAndRemark() {
    ...
    this.auditService.EndTaskAudit(payloadData).subscribe(
      (res) => {
        this.success(res);
        ...
        this.GetAuditTaskAvailablity();
        this.afterSubmitAudit.emit(res);
      ...
      (err) => {
        ...
        this.actionRemarkValue = "";
        this.GetAuditTaskAvailablity();
        this.afterSubmitAudit.emit(err);
        this.shownCrossbutton = false;
      }
    );
  }
  ...
  statusValueName: any;
  supportTypeAarry = ["On-Site", "Remote"];
  ...
  getStartServiceOrder(woId: any) {
    ...
    //console.log(data);
    this.auditService.StartTaskAudit(data).subscribe(
      (res) => {
        this.success(res);
        this.shownCrossbutton = false;
  ...
  createV2_MX_TechAttendanceProject_Start(task, value: any) {
    ...
    this.auditService.StartTechTaskAudit(requestData).subscribe(
      (res: any) => {
        this.success(res);
        this.GetAuditTaskAvailablity();
        this.afterSubmitAudit.emit(res);
  ...
  UpdateV2_MX_TechAttendanceProject_End(task, value: any) {
    ...
    this.auditService.EndTechTaskAudit(requestData).subscribe(
      (res: any) => {
        this.success(res);
        this.GetAuditTaskAvailablity();
        this.afterSubmitAudit.emit(res);
